...
contract RockPaperScissors {
    ...
    uint16 constant GAME_TIMEOUT = 300;
    ...
    event CommitRequest(uint gameID, address p1, address p2);
    ...
    modifier playerNotMoved(uint gameID) {
        ...
        if (game.p1 == msg.sender) {
            require(game.p1MoveHash == DEFAULT_HASH);
        }
        if (game.p2 == msg.sender) {
            require(game.p2MoveHash == DEFAULT_HASH);
        }
        _;
    }
    ...
    modifier bothPlayersMoved(uint gameID) {
        Game memory game = games[gameID];
        require(game.p1MoveHash != DEFAULT_HASH && game.p2MoveHash != DEFAULT_HASH);
        _;
    }
    ...
    function makeMove(uint gameID, bytes32 moveHash) payable public
        sentPayIn
        ...
        playerNotMoved(gameID)
    {
        ...
        if (msg.sender == game.p1) {
            game.p1MoveHash = moveHash;
        } else if (msg.sender == game.p2) {
            game.p2MoveHash = moveHash;
        }
        if (game.p1MoveHash != DEFAULT_HASH && game.p2MoveHash != DEFAULT_HASH) {
            game.timeout = now + GAME_TIMEOUT;
            emit CommitRequest(gameID, game.p1, game.p2);
        }
    }
        ...
        bothPlayersMoved(gameID)
    {
        ...
        if (game.p1Move != 3 && game.p2Move != 3) {
            winner = winnerEncoding[game.p1Move][game.p2Move];
            emit GameEnd(gameID, winner);
            
            if (winner == 0) {
                game.p1.transfer(STAKE);
                game.p2.transfer(STAKE);
            } else if (winner == 1) {
                game.p1.transfer(STAKE * 2);
            } else {
                game.p2.transfer(STAKE * 2);
            }
    ...
    function checkGameState(uint gameID) public {
        ...
        if (now > game.timeout) {
            if (game.p1MoveHash != DEFAULT_HASH && game.p2MoveHash != DEFAULT_HASH) {
                if (game.p1Move == 3 && game.p2Move != 3) {
                    game.p2.transfer(STAKE * 2);
                } else if (game.p2Move == 3 && game.p1Move != 3) {
                    game.p1.transfer(STAKE * 2);
                }
            }
            if (game.p1Move == 3 && game.p2Move == 3) {
